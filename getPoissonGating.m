function sext=getPoissonGating(baseline,dc,ac,freq,phase,latency,onset,offset,tau,T,N,kernel,kick,ramp_dc_flag,ramp_ac_flag)
...
if nargin<6, latency=0.1; end % ms
if nargin<7, onset=T(1); end % ms
if nargin<8, offset=T(end); end % ms
if nargin<9, tau=2; end % ms
if nargin<10, T=(0:.01:1000)'; end % ms
if nargin<11, N=1; end % number of target cells
if nargin<12, kernel=ones(1,N); end % connectivity to target cells
if nargin<13, kick=1; end
if nargin<14, ramp_dc_flag=0; end % whether to ramp dc from 0 to dc over [onset,offset], else step at onset
if nargin<15, ramp_ac_flag=0; end % whether to ramp ac from 0 to ac over [onset,offset], else step at onset
