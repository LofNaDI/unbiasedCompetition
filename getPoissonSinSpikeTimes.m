function [spikeevents,spiketimes,dynrate] = getPoissonSinSpikeTimes(tOn,tOff,latency_,freq_,normFreqSigma_,phase_,widthSigma,baseline,dc_,ac_,T,N)
  ...
  if nargin<11, T = (0:.01:1000)';        end
  if nargin<12, N = 1;                    end
  ...
  %   error('stop')
  % end
  [spiketimes,spikeevents] = nonhomPoissonSpikeTimesGenerator(dynrate/1e3,N,interval,dt); % dynrate in kHz
